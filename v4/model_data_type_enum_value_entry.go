/*
RevEng.AI API

RevEng.AI is an AI-powered binary analysis platform for reverse engineering and malware analysis. It provides similarity search across executable binaries and functions, AI-driven decompilation, dynamic execution analysis, firmware unpacking, and integration with external threat intelligence sources like VirusTotal.

*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package sdk

import (
	"encoding/json"
	"bytes"
	"fmt"
)

// checks if the DataTypeEnumValueEntry type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &DataTypeEnumValueEntry{}

// DataTypeEnumValueEntry struct for DataTypeEnumValueEntry
type DataTypeEnumValueEntry struct {
	// Constant name.
	Name string `json:"name"`
	// Constant value, a decimal integer as a string, with no leading zeros. A string because the value may be negative or exceed 64 unsigned bits, which a JSON number cannot carry safely.
	Value string `json:"value" validate:"regexp=^-?(0|[1-9][0-9]*)$"`
}

type _DataTypeEnumValueEntry DataTypeEnumValueEntry

// NewDataTypeEnumValueEntry instantiates a new DataTypeEnumValueEntry object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewDataTypeEnumValueEntry(name string, value string) *DataTypeEnumValueEntry {
	this := DataTypeEnumValueEntry{}
	this.Name = name
	this.Value = value
	return &this
}

// NewDataTypeEnumValueEntryWithDefaults instantiates a new DataTypeEnumValueEntry object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewDataTypeEnumValueEntryWithDefaults() *DataTypeEnumValueEntry {
	this := DataTypeEnumValueEntry{}
	return &this
}

// GetName returns the Name field value
func (o *DataTypeEnumValueEntry) GetName() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.Name
}

// GetNameOk returns a tuple with the Name field value
// and a boolean to check if the value has been set.
func (o *DataTypeEnumValueEntry) GetNameOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Name, true
}

// SetName sets field value
func (o *DataTypeEnumValueEntry) SetName(v string) {
	o.Name = v
}

// GetValue returns the Value field value
func (o *DataTypeEnumValueEntry) GetValue() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.Value
}

// GetValueOk returns a tuple with the Value field value
// and a boolean to check if the value has been set.
func (o *DataTypeEnumValueEntry) GetValueOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Value, true
}

// SetValue sets field value
func (o *DataTypeEnumValueEntry) SetValue(v string) {
	o.Value = v
}

func (o DataTypeEnumValueEntry) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o DataTypeEnumValueEntry) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	toSerialize["name"] = o.Name
	toSerialize["value"] = o.Value
	return toSerialize, nil
}

func (o *DataTypeEnumValueEntry) UnmarshalJSON(data []byte) (err error) {
	// This validates that all required properties are included in the JSON object
	// by unmarshalling the object into a generic map with string keys and checking
	// that every required field exists as a key in the generic map.
	requiredProperties := []string{
		"name",
		"value",
	}

	allProperties := make(map[string]interface{})

	err = json.Unmarshal(data, &allProperties)

	if err != nil {
		return err;
	}

	for _, requiredProperty := range(requiredProperties) {
		if _, exists := allProperties[requiredProperty]; !exists {
			return fmt.Errorf("no value given for required property %v", requiredProperty)
		}
	}

	varDataTypeEnumValueEntry := _DataTypeEnumValueEntry{}

	decoder := json.NewDecoder(bytes.NewReader(data))
	decoder.DisallowUnknownFields()
	err = decoder.Decode(&varDataTypeEnumValueEntry)

	if err != nil {
		return err
	}

	*o = DataTypeEnumValueEntry(varDataTypeEnumValueEntry)

	return err
}

type NullableDataTypeEnumValueEntry struct {
	value *DataTypeEnumValueEntry
	isSet bool
}

func (v NullableDataTypeEnumValueEntry) Get() *DataTypeEnumValueEntry {
	return v.value
}

func (v *NullableDataTypeEnumValueEntry) Set(val *DataTypeEnumValueEntry) {
	v.value = val
	v.isSet = true
}

func (v NullableDataTypeEnumValueEntry) IsSet() bool {
	return v.isSet
}

func (v *NullableDataTypeEnumValueEntry) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableDataTypeEnumValueEntry(val *DataTypeEnumValueEntry) *NullableDataTypeEnumValueEntry {
	return &NullableDataTypeEnumValueEntry{value: val, isSet: true}
}

func (v NullableDataTypeEnumValueEntry) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableDataTypeEnumValueEntry) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


