...
type ApiSubmitUserFeedbackRequest struct {
	ctx context.Context
	ApiService *AuthenticationUsersAPIService
	submitUserFeedbackRequest *SubmitUserFeedbackRequest
}

func (r ApiSubmitUserFeedbackRequest) SubmitUserFeedbackRequest(submitUserFeedbackRequest SubmitUserFeedbackRequest) ApiSubmitUserFeedbackRequest {
	r.submitUserFeedbackRequest = &submitUserFeedbackRequest
	return r
}

func (r ApiSubmitUserFeedbackRequest) Execute() (*BaseResponse, *http.Response, error) {
	return r.ApiService.SubmitUserFeedbackExecute(r)
}

/*
SubmitUserFeedback Submit feedback about the application
...
func (a *AuthenticationUsersAPIService) SubmitUserFeedbackExecute(r ApiSubmitUserFeedbackRequest) (*BaseResponse, *http.Response, error) {
	...
	localBasePath, err := a.client.cfg.ServerURLWithContext(r.ctx, "AuthenticationUsersAPIService.SubmitUserFeedback")
	if err != nil {
		return localVarReturnValue, nil, &GenericOpenAPIError{error: err.Error()}
	}

	localVarPath := localBasePath + "/v2/users/feedback"

	localVarHeaderParams := make(map[string]string)
	localVarQueryParams := url.Values{}
	localVarFormParams := url.Values{}
	if r.submitUserFeedbackRequest == nil {
		return localVarReturnValue, nil, reportError("submitUserFeedbackRequest is required and must be specified")
	}
	...
	// body params
	localVarPostBody = r.submitUserFeedbackRequest
	if r.ctx != nil {
		// API Key Authentication
		if auth, ok := r.ctx.Value(ContextAPIKeys).(map[string]APIKey); ok {
			if apiKey, ok := auth["APIKey"]; ok {
